...
dropdown::dropdown( int x, int y, std::string name, std::string var, std::vector< std::string > options ) {
	m_opos = m_pos = { x, y };
	m_size = { 155, 20 };
	m_name = name;
	m_var = var;
...
void dropdown::render( ) {
	...
	auto index = g_cfg [ m_var ].get<int>( );
	render::d3d::get().text( m_elements.at( index ), m_pos.x + 11, m_pos.y + 4, left, fonts::menu, false, COLOUR( 204, 204, 204 ) );
